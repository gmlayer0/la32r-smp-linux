...
static int rela_stack_push(s32 stack_value, s32 *rela_stack, size_t *rela_stack_top)
{
	if (*rela_stack_top >= RELA_STACK_DEPTH)
		return -ENOEXEC;

	rela_stack[(*rela_stack_top)++] = stack_value;
	pr_debug("%s stack_value = 0x%x\n", __func__, stack_value);

	return 0;
}

static int rela_stack_pop(s32 *stack_value, s32 *rela_stack, size_t *rela_stack_top)
{
	if (*rela_stack_top == 0)
		return -ENOEXEC;
...
static int apply_r_larch_none(struct module *me, u32 *location, Elf_Addr v,
				s32 *rela_stack, size_t *rela_stack_top)
{
	return 0;
}
...
static int apply_r_larch_32(struct module *me, u32 *location, Elf_Addr v,
				s32 *rela_stack, size_t *rela_stack_top)
{
	if ((unsigned int)v == 0xc0000060)
		pr_info("location = 0x%x, *location = 0x%x\n",
						...
						s32 *rela_stack, size_t *rela_stack_top)
{
	...
	/* check 5-bit signed */
	if ((opr1 & ~(u32)0xf) &&
	    (opr1 & ~(u32)0xf) != ~(u32)0xf) {
		pr_err("module %s: opr1 = 0x%x overflow! dangerous %s relocation\n",
						...
						s32 *rela_stack, size_t *rela_stack_top)
{
	...
	/* check 4-aligned */
	if (opr1 % 4) {
		pr_err("module %s: opr1 = 0x%x unaligned! dangerous %s relocation\n",
			me->name, opr1, __func__);
		return -ENOEXEC;
	}
						...
						s32 *rela_stack, size_t *rela_stack_top)
{
	...
	/* check 20-bit signed */
	if ((opr1 & ~(u32)0xfffff) &&
	    (opr1 & ~(u32)0xfffff) != ~(u32)0xfffff) {
		pr_info("opr1 & ~(u32)0x7ffff = 0x%x\n", (opr1 & ~(u32)0x7ffff));
		pr_err("module %s: opr1 = 0x%x overflow! dangerous %s relocation\n",
			me->name, opr1, __func__);
		return -ENOEXEC;
	}
							...
							s32 *rela_stack, size_t *rela_stack_top)
{
	...
	//opr1 >>= 2;
	/* check 23-bit signed */
	if ((opr1 & ~(u32)0x7fffff) &&
	    (opr1 & ~(u32)0x7fffff) != ~(u32)0x7fffff) {
		pr_err("module %s: opr1 = 0x%x overflow! dangerous %s relocation\n",
			me->name, opr1, __func__);
		return -ENOEXEC;
	}
							...
							s32 *rela_stack, size_t *rela_stack_top)
{
	...
	//opr1 >>= 2;
	/* check 28-bit signed */
	if ((opr1 & ~(u32)0xfffffff) &&
	    (opr1 & ~(u32)0xfffffff) != ~(u32)0xfffffff) {
		pr_err("module %s: opr1 = 0x%x overflow! dangerous %s relocation\n",
			me->name, opr1, __func__);
		return -ENOEXEC;
	}
		   ...
		   struct module *me)
{
	int i, err;
	unsigned int type;
	s32 rela_stack[RELA_STACK_DEPTH];
		       ...
		       struct module *me)
{
	...
	/* FIXME: test for vmalloc */
	int *my_ptr = NULL;
	my_ptr = vmalloc(sizeof(int));
	*my_ptr = 0x1;
	pr_info("=========my_ptr = 0x%x, *my_ptr = %d====\n", (int)my_ptr, *my_ptr);

	pr_info("%s: Applying relocate section %u to %u\n", __func__, relsec,
	       sechdrs[relsec].sh_info);
	...
	for (i = 0; i < sechdrs[relsec].sh_size / sizeof(*rel); i++) {
		...
		type = ELF_R_TYPE(rel[i].r_info);
		pr_info("SJY type = %d, line %d func %s\n", type, __LINE__, __func__);
		...
		pr_info("type %d st_value %x r_addend %x loc %x\n",
		       (int)ELF32_R_TYPE(rel[i].r_info),
		       sym->st_value, rel[i].r_addend, (u32)location);

		v = sym->st_value + rel[i].r_addend;
		pr_info("st_value = 0x%lx, st_name = %d, r_addend = 0x%lx\n",
			(unsigned long)sym->st_value, (unsigned)sym->st_name,
			(unsigned long)rel[i].r_addend);
		pr_info("v = 0x%lx, location = 0x%lx\n",
			(unsigned long)v, (unsigned long)location);
		err = handler(me, location, v, rela_stack, &rela_stack_top);
		if (err)
			return err;
	}
