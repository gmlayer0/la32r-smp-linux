...
#endif
#define dbg(host, channels, args...)		  dev_dbg(&host->pdev->dev, args);
...
static int ls2k_mci_prepare_dma(struct ls2k_mci_host *host, struct mmc_data *data)
{
	...
	dma_order = (readl(dma_order_reg) & 0xfUL) | ((host->sg_dma) | 0x8UL);

	/*sdio use dma1 write status back*/
	dma_order |= 0x1UL;
	dma_order |= 0x4UL;
	writel(dma_order, dma_order_reg);
...
static int ls2k_mci_probe(struct platform_device *pdev)
{
	...
	if (!(mmc->caps & MMC_CAP_NONREMOVABLE) &&
		...
		local_irq_save(flags);
		val = readl((void *)cd_gpio_inten_base);
		val |= (0x1 << cd_gpio_num);
		writel(val, (void *)cd_gpio_inten_base);
		local_irq_restore(flags);
